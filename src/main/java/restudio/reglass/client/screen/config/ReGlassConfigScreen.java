...
public class ReGlassConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        previewCircle = addRenderableWidget(new LiquidGlassWidget(previewX, previewY, 100, 100, s1).setCornerRadiusPx(50f));

        WidgetStyle s2 = WidgetStyle.create().tint(cfg.defaultTintColor, cfg.defaultTintAlpha).blurRadius(cfg.defaultBlurRadius)
                .shadow(cfg.defaultShadowExpand, cfg.defaultShadowFactor, cfg.defaultShadowOffsetX, cfg.defaultShadowOffsetY)
                .shadowColor(cfg.defaultShadowColor, cfg.defaultShadowColorAlpha);

        previewRounded = addRenderableWidget(new LiquidGlassWidget(previewX + 110, previewY + 20, 140, 60, s2).setCornerRadiusPx(16f));
    }
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredString(this.font, Component.literal("ReGlass Config (Scrollable)"), this.width / 2, 15, 0xFFFFFFFF);
    }
    ...
    private class TitleWidget extends AbstractWidget {
        ...
        public void renderWidget(GuiGraphics context, int mouseX, int mouseY, float delta) {
            if (this.visible) {
                context.drawCenteredString(ReGlassConfigScreen.this.font, this.getMessage(), this.getX() + this.getWidth() / 2, this.getY() + (this.getHeight() - 8) / 2, 0xFFFFFF);
            }
        }
